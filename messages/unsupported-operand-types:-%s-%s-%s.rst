...
.. meta::
	:description:
		Unsupported operand types: %s %s %s: This error is reported when the types of the operands are valid (here, ``int`` and ``string``), but they are not compatible one another.
	:og:image: https://php-errors.readthedocs.io/en/latest/_static/logo.png
	:og:type: article
	:og:title: Unsupported operand types: %s %s %s
	:og:description: This error is reported when the types of the operands are valid (here, ``int`` and ``string``), but they are not compatible one another
